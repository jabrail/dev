...
package  silin.utils
{
	...
	public class Color
	{
		...
		static public function mixColors(clr1:uint, clr2:uint, ratio:Number = 0.5):uint
		{
			...
			var r:int = int((1 - ratio) * (clr1 >> 16 & 0xFF) + ratio * (clr2 >> 16 & 0xFF));
			var g:int = int((1 - ratio) * (clr1 >> 8 & 0xFF) + ratio * (clr2 >> 8 & 0xFF));
			var b:int = int((1 - ratio) * (clr1  & 0xFF) + ratio * (clr2  & 0xFF));
